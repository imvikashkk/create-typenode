...
function validateProjectName(name) {
  ...
  if (!/^[a-z0-9-_.]+$/i.test(name)) {
    ...
    log('📋 Use only letters, numbers, hyphens, underscores, and dots', 'yellow');
    return false;
  }
...
function promptForProjectName(rl) {
  return new Promise((resolve) => {
    ...
    rl.question(`${colors.cyan}📁 Project location: ${colors.reset}`, (answer) => {
...
function copyTemplate(templatePath, targetPath, projectName, isCurrentDir = false) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (entry.name === 'package.json') {
          ...
          content = content.replace(/"name": "createtypenode"/, `"name": "${actualProjectName}"`);
        }
...
function promptForCurrentDirInstall(rl) {
  return new Promise((resolve) => {
    ...
    rl.question(`${colors.cyan}Continue anyway? (y/N): ${colors.reset}`, (answer) => {
...
${colors.blue}📁 Location: ${colors.bold}${isCurrentDir ? 'Current directory' : projectName}${colors.reset}
...
${colors.blue}📚 Available Scripts:${colors.reset}
  ${colors.green}npm run dev${colors.reset}         - Development with hot reload
...
async function main() {
  ...
  if (projectName === '.') {
    ...
    if (fs.existsSync(targetPath)) {
      ...
      log('💡 Please choose a different name or remove the existing directory', 'yellow');
      if (rl) rl.close();
      process.exit(1);
    }
